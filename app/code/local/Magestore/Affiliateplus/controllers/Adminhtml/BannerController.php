...
class Magestore_Affiliateplus_Adminhtml_BannerController extends Mage_Adminhtml_Controller_Action
{
	...
	public function saveAction() {
		...
		if ($data = $this->getRequest()->getPost()) {
			...
			try {
				$banner->save();
				Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('affiliateplus')->__('The banner has been saved successfully.'));
        ...
        Mage::getSingleton('adminhtml/session')->addError(Mage::helper('affiliateplus')->__('Unable to find a banner to save'));
        $this->_redirect('*/*/', array('store' => $storeId));
	}
	...
	public function deleteAction() {
		...
		if( $bannerId > 0 ) {
			try {
				...
				Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('adminhtml')->__('The banner has been deleted successfully.'));
				$this->_redirect('*/*/', array('store' => $storeId));
			} catch (Exception $e) {
				Mage::getSingleton('adminhtml/session')->addError($e->getMessage());

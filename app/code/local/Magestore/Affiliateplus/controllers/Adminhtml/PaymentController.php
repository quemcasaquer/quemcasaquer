...
class Magestore_Affiliateplus_Adminhtml_PaymentController extends Mage_Adminhtml_Controller_Action {
    ...
    protected function _initAction() {
        $this->loadLayout()
                ...
                ->_addBreadcrumb(Mage::helper('adminhtml')->__('Withdrawal Manager'), Mage::helper('adminhtml')->__('Withdrawal Manager'));
...
//				->setTransactionId($paypalPayment->getTransactionId());
        ...
        if ($payment->getId() || $id == 0) {
            ...
            $this->_addBreadcrumb(Mage::helper('adminhtml')->__('Payment Manager'), Mage::helper('adminhtml')->__('Withdrawal Manager'));
            $this->_addBreadcrumb(Mage::helper('adminhtml')->__('New Payment'), Mage::helper('adminhtml')->__('New Withdrawal'));
            ...
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('affiliateplus')->__('This item does not exist.'));
            $this->_redirect('*/*/');
        }
    }
    ...
    public function reviewAction() {
        ...
        if ($data = $this->getRequest()->getPost()) {
            ...
            if (!$paymentId && $amount < Mage::helper('affiliateplus/config')->getPaymentConfig('payment_release', $storeId)) {
                Mage::getSingleton('adminhtml/session')->addError($this->__('The minimum balance allowing withdrawal request is %s', Mage::helper('core')->currency(Mage::helper('affiliateplus/config')->getPaymentConfig('payment_release', $storeId), true, false)));
...
//                }
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('affiliateplus')->__('Your withdrawal request has been saved.'));
    ...
    public function deleteAction() {
        ...
        if ($this->getRequest()->getParam('id') > 0) {
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('adminhtml')->__('Your withdrawal request has been deleted.'));
                $this->_redirect('*/*/');
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
    ...
    public function processpaymentAction() {
        ...
        if ($account->getStatus() == 2) {
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('affiliateplus')->__('This account has been disabled.'));
            $this->_redirect('*/*/edit', array('id' => $paymentId));
            return;
        }
...
//                    $payment->sendMailProcessPaymentToAccount();
            ...
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('affiliateplus')->__('The payment is being processed'));
        } else {
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('affiliateplus')->__('This payment is completed successfully.'));
        }

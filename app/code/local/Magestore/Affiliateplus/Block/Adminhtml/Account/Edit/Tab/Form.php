	...
	extends Mage_Adminhtml_Block_Widget_Form
{
  protected function _prepareForm()
  {
      ...
      $fieldset->addField('email', 'text', array(
          'label'     => Mage::helper('affiliateplus')->__('Email Address'),
          'name'      => 'email',
		  //'readonly'  => 'readonly',
      ...
      if (!isset($data['account_id']) || !$data['account_id']){
		  ...
		  'name'	=> 'password',
		  'note'	=> Mage::helper('affiliateplus')->__('Password can be changed by an affiliate.'),
		));
      } else {
      	$fieldset->addField('customer_account','note',array(
      		'label'	=> Mage::helper('affiliateplus')->__('Customer Account'),
             ...
             /*
          hainh update show Referring WebSite
          22-04-2014
         */
      ...
      $fieldset->addField('notification', 'select', array(
          'label'     => Mage::helper('affiliateplus')->__('Receive notification emails'),
          'required'  => false,
          'name'      => 'notification',
	  ...
	  if($data && isset($data['customer_id']) && $data['customer_id']){
          ...
          $fieldset->addField('associate_website_id', 'select', array(
              ...
              'note'    => Mage::helper('affiliateplus')->__('For creating a new affiliate account which does not exist in customer database')
          ));
       }

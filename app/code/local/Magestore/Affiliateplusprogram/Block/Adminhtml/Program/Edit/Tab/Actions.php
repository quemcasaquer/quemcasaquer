...
class Magestore_Affiliateplusprogram_Block_Adminhtml_Program_Edit_Tab_Actions extends Mage_Adminhtml_Block_Widget_Form
{
  protected function _prepareForm()
  {
      ...
      $fieldset->addField('sec_commission', 'select', array(
          ...
          'after_element_html' => '<p class="note">' . Mage::helper('affiliateplusprogram')->__('Select "No" to apply the above commission to all orders') . '</p>' .
      ...
      $fieldset->addField('customer_group_ids', 'multiselect', array(
          'label'     => Mage::helper('affiliateplusprogram')->__('Customer Groups Applied'),
          'required'  => false,
          'values'    => Mage::getResourceModel('customer/group_collection')->load()
							->toOptionArray(),
          'name'      => 'customer_group_ids',
          'disabled'  => ($inStore && !$data['customer_group_ids_in_store']),
          'after_element_html' => '<p class="note">' . Mage::helper('affiliateplusprogram')->__('Discount is applied to customers who buy products directly. Select none for all customers.') . '</p>' .
          ($inStore ? '</td><td class="use-default">

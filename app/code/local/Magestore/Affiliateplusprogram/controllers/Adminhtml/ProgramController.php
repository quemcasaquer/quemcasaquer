                ...
                if (isset($data['autojoin']) && $data['autojoin'] && isset($data['scope']) && $data['scope'] == Magestore_Affiliateplusprogram_Model_Scope::SCOPE_GLOBAL) {
                    ...
                    if ($model->getId()) {
                        $collections->getSelect()
                                ->joinLeft(array('j' => $collections->getTable('affiliateplusprogram/joined')), 'main_table.account_id = j.account_id AND j.program_id = ' . $model->getId(), array()
                                )->where('j.id IS NULL');
                    }
                    ...
                    if ($model->getData('customer_groups')) {
                        ...
                        $collections = Mage::getResourceModel('affiliateplus/account_collection');
                        $collections->getSelect()
                                ->joinLeft(array('c' => $collections->getTable('customer/entity')), 'main_table.customer_id = c.entity_id', array()

...
class AW_Followupemail_Block_Adminhtml_Linktracking_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
    ...
    protected function _prepareCollection()
    {
        $collection = Mage::getResourceModel('followupemail/linktracking_collection')
                        ->getLinktrackingCollection(AW_Followupemail_Helper_Data::getLinktrackingQueryType());
...
// adding cart-related fields
        if(AW_Followupemail_Model_Source_Linktracking_Types::LINKTRACKING_TYPE_LINK_CART == $this->_queryType)
        {
...
// adding order-related fields
        if(AW_Followupemail_Model_Source_Linktracking_Types::LINKTRACKING_TYPE_LINK_CART_ORDER == $this->_queryType)
        {
            $this->addColumn('quote_created_at',
                array(
                    'index'  => 'quote_created_at',
            ...
            if(AW_Followupemail_Helper_Data::getMagentoVersionCode() == AW_Followupemail_Helper_Data::MAGENTO_VERSION_CE_1_3)
...
// = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = service
    ...
    {
        $res = AW_Followupemail_Model_Mysql4_Linktracking_Collection::getRealFieldName($name);
        if(!$res)
        {
    ...
    protected function _filterConditionDate($collection, $column)
    {
        ...
        if(isset($value['from']) && isset($value['to']))
            $collection->getSelect()->where($this->getRealFieldName($column->getIndex()).' BETWEEN \''.
                date(AW_Followupemail_Model_Mysql4_Queue::MYSQL_DATETIME_FORMAT, $value['from']->get(Zend_Date::TIMESTAMP))
                .'\' AND \''.
                date(AW_Followupemail_Model_Mysql4_Queue::MYSQL_DATETIME_FORMAT, $value['to']->get(Zend_Date::TIMESTAMP))
                .'\'');
        elseif(isset($value['from']))
            $collection->getSelect()->where($this->getRealFieldName($column->getIndex()).'>\''.date(AW_Followupemail_Model_Mysql4_Queue::MYSQL_DATETIME_FORMAT, $value['from']->get(Zend_Date::TIMESTAMP)).'\'');
        elseif(isset($value['to']))
            $collection->getSelect()->where($this->getRealFieldName($column->getIndex()).'<\''.date(AW_Followupemail_Model_Mysql4_Queue::MYSQL_DATETIME_FORMAT, $value['to']->get(Zend_Date::TIMESTAMP)).'\'');
    }

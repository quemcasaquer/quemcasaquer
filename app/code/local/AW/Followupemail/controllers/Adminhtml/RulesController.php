...
class AW_Followupemail_Adminhtml_RulesController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function editAction()
    {
        ...
        if(!empty($data) || $ruleId == 0)
        {
            ...
            if(     !isset($data['customer_groups'])
                ...
                ||  (in_array(AW_Followupemail_Model_Source_Customer_Group::CUSTOMER_GROUP_ALL, $data['customer_groups'])
            ...
            if(Mage::helper('followupemail')->canUseAN())
            {
                ...
                ||  (   in_array(AW_Followupemail_Model_Mysql4_Rule::ADVANCED_NEWSLETTER_SEGMENTS_ALL, $data['anl_segments'])
            ...
            &&  $data['sale_amount']
            &&  is_array($saleAmount = explode(AW_Followupemail_Model_Source_Rule_Saleamount::CONDITION_SEPARATOR, $data['sale_amount'], 2))
            &&  count($saleAmount)>1
            ) {

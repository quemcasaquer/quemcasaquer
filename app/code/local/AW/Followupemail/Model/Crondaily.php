...
class AW_Followupemail_Model_Crondaily
{
    /*
    ...
    public function cronJobs()
    {
        ...
        if(!$lastExecTime = $config->getParam(AW_Followupemail_Model_Config::LAST_EXEC_TIME_DAILY))
        {
            $config->setParam(AW_Followupemail_Model_Config::LAST_EXEC_TIME_DAILY, time());
            if(AW_Followupemail_Model_Cron::DEBUG_MODE)
                AW_Followupemail_Model_Log::log('CD Never runs');
            return;
        }
        ...
        ||  $now - $lastExecTime < $this->cronMinInterval
            ...
            if(AW_Followupemail_Model_Cron::DEBUG_MODE)
                AW_Followupemail_Model_Log::log('CD Already running or minimal interval isn\'t match '.date('d.m.Y H:i:s', $lastExecTime).' - '.($this->cronMinInterval - $now - $lastExecTime));
            return;
        }
        ...
        try
        {
            ...
            AW_Followupemail_Model_Log::log('cron daily started, last execution time is '
                .date(AW_Followupemail_Model_Mysql4_Queue::MYSQL_DATETIME_FORMAT, 
                        $lastExecTime + $timeShift));

            $this->_checkEvents();

            $config->setParam(AW_Followupemail_Model_Config::LAST_EXEC_TIME_DAILY, $now);
        }
        catch (Exception $e) { Mage::logException($e); }

        self::$_isRunning = false;
        AW_Followupemail_Model_Log::log('Daily cron stopped ');
    }
    ...
    protected function _checkBirthdays()
    {
        ...
        $ruleIds = Mage::getModel('followupemail/mysql4_rule')
                ->getRuleIdsByEventType(AW_Followupemail_Model_Source_Rule_Types::RULE_TYPE_CUSTOMER_BIRTHDAY);
            ...
            foreach(unserialize($rule->getChain()) as $chain)
            {
                ...
                if(count($birthDays))
                {
                    ...
                    foreach($birthDays as $birthDay)
                    {
                        ...
                        AW_Followupemail_Model_Log::log('customer birthday event processing, rule_id='.$ruleId.', customerId='.$params['customer_id'].', dob='.$params['dob'].', store_id='.$params['store_id']);

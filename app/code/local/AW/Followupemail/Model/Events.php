...
class AW_Followupemail_Model_Events
{
    const NEWSLETTER_SUBSCRIPTION_PROCESSED_FLAG = 'AW_fue_customer_subscription_already_processed';
    ...
    */
    public function wishlistProductAdd($eventData)
    {
        ...
        $queue->cancelByEvent(
            ...
            AW_Followupemail_Model_Source_Rule_Types::RULE_TYPE_WISHLIST_PRODUCT_ADD,
            $product->getId()
        );

        if (count($ruleIds = Mage::getModel('followupemail/mysql4_rule')
            ->getRuleIdsByEventType(AW_Followupemail_Model_Source_Rule_Types::RULE_TYPE_WISHLIST_PRODUCT_ADD))
        ) {
    ...
    public function wishlistShared($eventData)
    {
        ...
        $queue->cancelByEvent(
            $queue->getEmailByCustomerId($wishlist->getCustomerId()),
            AW_Followupemail_Model_Source_Rule_Types::RULE_TYPE_WISHLIST_SHARED,
            $wishlist->getWishlistId()
        );

        if (count($ruleIds = Mage::getModel('followupemail/mysql4_rule')
            ->getRuleIdsByEventType(AW_Followupemail_Model_Source_Rule_Types::RULE_TYPE_WISHLIST_SHARED))
        ) {
            $store = $wishlist->getStore();
    ...
    {
        if ($param instanceof AW_Followupemail_Model_Queue) $queue = $param;
        ...
        $queueResource->cancelByEvent(
            ...
            AW_Followupemail_Model_Source_Rule_Types::RULE_TYPE_CUSTOMER_CAME_BACK_BY_LINK,
            $queue->getObjectId()
        );

        if (count($ruleIds = Mage::getModel('followupemail/mysql4_rule')
            ->getRuleIdsByEventType(AW_Followupemail_Model_Source_Rule_Types::RULE_TYPE_CUSTOMER_CAME_BACK_BY_LINK))
        ) {
    ...
    public function customerAfterSave($event)
    {
        ...
        if ($customer->getData('group_id') != $customer->getOrigData('group_id') && $customer->getOrigData('group_id')) {
            ...
            $ruleIds = Mage::getModel('followupemail/mysql4_rule')
                ->getRuleIdsByEventType(AW_Followupemail_Model_Source_Rule_Types::RULE_TYPE_CUSTOMER_GROUP_CHANGED);
            if (count($ruleIds)) {
                $params = array(
    ...
    public function newsletterSubscribe($event)
    {
        ...
        if (Mage::helper('followupemail')->canUseAN())
            if (class_exists('AW_Advancednewsletter_Model_Subscriber'))
                if ($subscriber instanceof AW_Advancednewsletter_Model_Subscriber) {
                    $status_key = 'status';
                    $email_key = 'email';
                }
        $queue->cancelByEvent($subscriber->getData($email_key),
            AW_Followupemail_Model_Source_Rule_Types::RULE_TYPE_CUSTOMER_UNSUBSCRIPTION);
        $ruleIds = Mage::getModel('followupemail/mysql4_rule')
            ->getRuleIdsByEventType(AW_Followupemail_Model_Source_Rule_Types::RULE_TYPE_CUSTOMER_NEW_SUBSCRIPTION);
        if (count($ruleIds)) {
            $params = array(

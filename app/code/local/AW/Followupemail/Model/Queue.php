...
class AW_Followupemail_Model_Queue extends Mage_Core_Model_Abstract
{
    /*
     ...
     * @param string $code
     * @return AW_Followupemail_Model_Queue Self instance
     */
     ...
     * @see AW_Followupemail_Helper_Data::packArray()
    ...
    {
        $scheduledAt = date(AW_Followupemail_Model_Mysql4_Queue::MYSQL_DATETIME_FORMAT, $scheduledAt);

        $this
            ->setId(null)
            ->setCode($code)
            ->setCreatedAt(date(AW_Followupemail_Model_Mysql4_Queue::MYSQL_DATETIME_FORMAT, time()))
    ...
    {
        $this->setStatus('C')
            ->setSentAt(date(AW_Followupemail_Model_Mysql4_Queue::MYSQL_DATETIME_FORMAT, time()))
            ->save();
        return $this;
    }
  ...
  /*  public function send()
    {
        ...
        $customSMTPSettings = AW_Followupemail_Helper_Data::getCustomSMTPSettings();
        if(!($customSMTPSettings === FALSE)) {
            $customSMTPHost = Mage::getStoreConfig(AW_Customsmtp_Helper_Config::XML_PATH_SMTP_HOST);

...
class AW_Followupemail_Model_Mysql4_Linktracking_Collection extends Mage_Core_Model_Mysql4_Collection_Abstract 
{
    ...
    public function getLinktrackingCollection($queryType, $fields=false)
    {
        ...
        switch($queryType)
        {
            case AW_Followupemail_Model_Source_Linktracking_Types::LINKTRACKING_TYPE_LINK_CART :
                $this->getSelect()
                    ->columns(self::$cartFields)
                    ->joinInner(array('q' => $this->getTable('sales/quote')),
                                'q.entity_id=e.object_id', '')
                    ->where('r.event_type="'.AW_Followupemail_Model_Source_Rule_Types::RULE_TYPE_ABANDONED_CART_NEW.'"')
                    ->where('q.is_active=1');
                break;

            case AW_Followupemail_Model_Source_Linktracking_Types::LINKTRACKING_TYPE_LINK_CART_ORDER :

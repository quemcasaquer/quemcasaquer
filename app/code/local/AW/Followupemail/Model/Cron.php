...
class AW_Followupemail_Model_Cron
{
    ...
    {        
        ...
        if (self::DEBUG_MODE) AW_Followupemail_Model_Log::log('Removing old coupons');
        $timeShift = Mage::app()->getLocale()->date()->get(Zend_Date::TIMEZONE_SECS);
        ...
        if (self::DEBUG_MODE) AW_Followupemail_Model_Log::log(sprintf("Total %d old coupons found", $collection->getSize()));
        ...
        if (self::DEBUG_MODE) AW_Followupemail_Model_Log::log('Completed removing old coupons');
    }
    ...
    {
        if (self::DEBUG_MODE) AW_Followupemail_Model_Log::log("Processing Order Status History");
        ...
        while ($currentPage <= $_pages) {
            if (self::DEBUG_MODE) AW_Followupemail_Model_Log::log("Processing page {$currentPage} from {$_pages} total");
    ...
    {
        if (self::DEBUG_MODE) AW_Followupemail_Model_Log::log("Checking abandoned carts");
        $resource = Mage::getSingleton('core/resource');
        $read = $resource->getConnection('core_read');
        ...
        $select = $read->select()
            ...
            ->where('q.is_active=1')
            ->where('q.updated_at > ?', date(AW_Followupemail_Model_Mysql4_Queue::MYSQL_DATETIME_FORMAT,

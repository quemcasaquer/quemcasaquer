...
class AW_Followupemail_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    /**
     * If AW_Customsmtp installed and not disabled and if value of option 'Send
    ...
    public static function getCustomSMTPSettings()
    {
        ...
        if (array_key_exists('AW_Customsmtp', $_modules) &&
            ...
            Mage::getStoreConfig(AW_Customsmtp_Helper_Config::XML_PATH_ENABLED) &&
            Mage::getStoreConfig('customsmtp/general/mode') == AW_Customsmtp_Model_Source_Mode::ON
        ) {
            $customSMTPSettings = array(
                'port' => Mage::getStoreConfig(AW_Customsmtp_Helper_Config::XML_PATH_SMTP_PORT),
            ...
            if (!empty($needSSL)) {
                $customSMTPSettings['ssl'] = strtolower(Mage::getStoreConfig(AW_Customsmtp_Helper_Config::XML_PATH_SMTP_SSL));
            }
    ...
    public static function isMSSInstalled()
    {
        ...
        return array_key_exists('AW_Marketsuite', $modules)
            ...
            && !(bool)Mage::getStoreConfig('advanced/modules_disable_output/AW_Marketsuite');
    }
    ...
    public function canUseAN()
    {
        if ($this->_canUseAN === null) {
            $modules = (array)Mage::getConfig()->getNode('modules')->children();
            $_isInstalled = array_key_exists('AW_Advancednewsletter', $modules)
                && 'true' == (string)$modules['AW_Advancednewsletter']->active
                && !(bool)Mage::getStoreConfig('advanced/modules_disable_output/AW_Advancednewsletter');
            $this->_canUseAN = $_isInstalled && version_compare($modules['AW_Advancednewsletter']->version, self::FUE_AN_MINVERSION, '>=');
        }
    ...
    public static function getLinktrackingQueryType()
    {
        ...
        if (!$queryType)
            $queryType = AW_Followupemail_Model_Source_Linktracking_Types::LINKTRACKING_TYPE_LINK_ONLY;

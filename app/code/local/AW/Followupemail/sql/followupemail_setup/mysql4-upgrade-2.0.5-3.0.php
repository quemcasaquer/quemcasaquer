...
INSERT INTO $configTableName VALUES('".AW_Followupemail_Model_Config::LAST_EXEC_TIME."', '".$lastExecTime."');
INSERT INTO $configTableName VALUES('".AW_Followupemail_Model_Config::LAST_EXEC_TIME_DAILY."', '".((int)$lastExecTime-3600)."');
INSERT INTO $configTableName VALUES('".AW_Followupemail_Model_Config::EMAIL_SENDER_LAST_EXEC_TIME."', '".$lastExecTime."');
");
...
$ruleTypeConvert = array('abandoned_cart' => AW_Followupemail_Model_Source_Rule_Types::RULE_TYPE_ABANDONED_CART_NEW);
foreach(Mage::getSingleton('sales/order_config')->getStatuses() as $code => $name)
    $ruleTypeConvert[$code] = AW_Followupemail_Model_Source_Rule_Types::RULE_TYPE_ORDER_STATUS_PREFIX . $code;
...
foreach($oldRules as $rule)
{
    ...
    foreach($chain as $k => $v)
    {
        ...
        $chain[$k]['TEMPLATE_ID'] = AW_Followupemail_Model_Source_Rule_Template::TEMPLATE_SOURCE_EMAIL
            .AW_Followupemail_Model_Source_Rule_Template::TEMPLATE_SOURCE_SEPARATOR
    ...
    $installer->run('INSERT INTO '.$ruleTableName.' SET '
        ...
        .', product_type_ids="'.($rule['PRODUCT_TYPE']?$rule['PRODUCT_TYPE']:AW_Followupemail_Model_Source_Product_Types::PRODUCT_TYPE_ALL).'"'

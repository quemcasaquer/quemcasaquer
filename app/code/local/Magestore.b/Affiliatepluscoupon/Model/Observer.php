...
class Magestore_Affiliatepluscoupon_Model_Observer
{
	...
	public function getAffiliateInfo($observer){
		...
		if (!$affilateData || !is_array($affilateData) || !isset($affilateData['program_id'])) return $this;
	...
	public function addAccountTab($observer){
		if (!$observer->getEvent()->getId()) return $this;
	...
	public function afterSaveAccount($observer){
		...
		if (!isset($data['account_coupon'])) return $this;
		if (!$data['account_coupon']) return $this;
		
		$programCoupons = array();
		parse_str(urldecode($data['account_coupon']),$programCoupons);
		if (!count($programCoupons)) return $this;
		...
		foreach ($programCoupons as $pId => $enCoded){
			...
			if (isset($codeArr['coupon_code'])) $code = $codeArr['coupon_code'];
			if ($coupon->getCouponCode() == $code || !$code) continue;
			try{
		...
		$fieldset->addField('use_coupon','select',array(
			...
			'after_element_html'	=> '<script type="text/javascript">
				function changeCouponOption(el){
					if (el.value == 1)
						$(\'coupon_pattern\').parentNode.parentNode.show();
					else
						$(\'coupon_pattern\').parentNode.parentNode.hide();
				}
				Event.observe(window,\'load\',function(){
					changeCouponOption($(\'use_coupon\'));
				});
			</script>',
		));
	...
	public function addFieldTransactionForm($observer){
		...
		if (!isset($transactionData['coupon_code']) || !$transactionData['coupon_code']) return $this;
		$fieldset = $observer->getEvent()->getFieldset();

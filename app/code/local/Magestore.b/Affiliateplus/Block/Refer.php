<?php
class Magestore_Affiliateplus_Block_Refer extends Mage_Core_Block_Template
{
	/**
	 * get Helper
	 *
	...
	public function getPrefixUrl(){
		return $this->getBaseUrl() . $this->_getHelper()->getReferConfig('url_prefix');
	}
	...
	public function getTrafficSources(){
		...
		$referers = Mage::getResourceModel('affiliateplus/referer_collection')->addFieldToFilter('account_id',$accountId);
		$trafficSources = array('facebook' => 0, 'twitter' => 0, 'google' => 0, 'email' => 0);
		foreach ($referers as $refer){
			$referer = $refer->getData('referer');
			if ($this->getPersonalPath() && strpos($this->getPersonalPath(),trim($refer->getData('url_path'),'/'))===false) continue;

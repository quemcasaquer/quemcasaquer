		...
		if ($data = $this->getRequest()->getPost()) {
			...
			try {
				...
				if (isset($data['autojoin']) && $data['autojoin'] && isset($data['scope']) && $data['scope'] == Magestore_Affiliateplusprogram_Model_Scope::SCOPE_GLOBAL ){
					$data['program_account'] = Mage::getResourceModel('affiliateplus/account_collection')->getAllIds();
					...
					if ($model->getData('customer_groups')){
						...
						$accountIds = array();
						$accountCollection = Mage::getResourceModel('affiliateplus/account_collection');
						$customerModel = Mage::getModel('customer/customer');
						foreach ($accountCollection as $account){
							$customerModel->load($account->getCustomerId());
							if (in_array($customerModel->getGroupId(),$customerGroups))
								$accountIds[] = $account->getId();
						}
						$data['program_account'] = $accountIds;
					}
				}elseif (isset($data['program_account']) && is_string($data['program_account'])) {
					$programAccount = array();
					parse_str($data['program_account'],$programAccount);
					$data['program_account'] = array_unique(array_keys($programAccount));
				}

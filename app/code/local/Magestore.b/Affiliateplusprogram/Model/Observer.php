	...
	public function accountSaveAfter($observer){
		...
		if ($affiliateplusAccount->hasData('account_program')){
			...
			foreach ($oldProgramCollection as $oldProgram){
				...
				if (in_array($oldProgram->getProgramId(),$joinPrograms)) continue;
			...
			$oldProgramCollection = Mage::getResourceModel('affiliateplusprogram/account_collection')
				->addFieldToFilter('account_id',$affiliateplusAccount->getId());
			if ($oldProgramCollection->getSize()) return $this;
			...
			if ($account = $info['account']){
				...
				if ($program){
					...
					if ($program->getDiscountType() == 'fixed')
						$price -= floatval($program->getDiscount());
					elseif ($program->getDiscountType() == 'percentage')
						$price -= floatval($program->getDiscount()) / 100 * $price;
		...
		foreach ($affiliateInfo as $info)
			if ($account = $info['account']){
				if ($account->getUsingCoupon()){
					...
					if (!$program->validateOrder($address->getQuote())) return $this;
				}
				foreach ($items as $item){
					if ($account->getUsingCoupon()){
						if (!$program->validateItem($item)) continue;
					}else {
		...
		foreach ($affiliateInfo as $info)
			if ($account = $info['account']){
				if ($account->getUsingCoupon()){
					...
					if (!$program->validateOrder($order)) return $this;
				}
				foreach ($items as $item){
					if ($account->getUsingCoupon()){
						if (!$program->validateItem($item)) continue;
					...
					if ($program){
						$affiliateType = $program->getAffiliateType() ? $program->getAffiliateType() : $this->_getConfigHelper()->getGeneralConfig('affiliate_type');
						...
						if ($program->getCommissionType() == 'fixed'){
							...
							//$itemCommission = $item->getQtyOrdered() * min($commissionValue, $baseProfit);
						}elseif ($program->getCommissionType() == 'percentage'){
							...
							//$itemCommission = $item->getQtyOrdered() * $baseProfit * $commissionValue / 100;
						}
						if ($item->getHasChildren()) $itemCommission = 0;
						...
						$commission += $commissionObject->getCommission();//$itemCommission;
						...
						$extraContent[$program->getId()]['commission'] += $commissionObject->getCommission();//$itemCommission;
					}
				}
